...
namespace VNPay_Web.Report
{
    public partial class WorkResult : PagePermisstion
    {
        ...
        protected void lbKPI_Click(object sender, EventArgs e)
        {
            ...
            LinkButton lbSURVEYPOSM = item.FindControl("lbSURVEYPOSM") as LinkButton;
          
            string CommandName = lk.CommandName;

            List<LinkButton> linkButtons = new List<LinkButton>()
            {
                lbATT, lbPRO, lbSURVEYPOSM
            };
            int KPIId = 0;
        ...
        private void CheckVisibleTabByWorkId(int WorkId, RepeaterItem item)
        {
            ...
            HtmlGenericControl li_surveyposm = item.FindControl("li_surveyposm") as HtmlGenericControl;
            ...
            li_promotion.Visible = li_surveyposm.Visible = false;
            if (data != null && data.Rows.Count > 0)
            {
                foreach (DataRow dr in data.Rows)
                {
                    switch (Convert.ToInt32(dr["KPIId"]))
                    {
                        ...
                        case (int)KPI.PROMOTION: li_promotion.Visible = true; lbPRO.Text = Convert.ToString(dr["KPIName"]); lbPRO.CommandArgument = Convert.ToString(dr["KPIId"]); break;
                        case (int)KPI.SURVEYPOSM: li_surveyposm.Visible = true; lbSURVEYPOSM.Text = Convert.ToString(dr["KPIName"]); lbSURVEYPOSM.CommandArgument = Convert.ToString(dr["KPIId"]); break;
        ...
        private void ExportRawData()
        {
            ...
            try
            {
                ...
                if ((KPIId >= 1 && KPIId <= 5) || KPIId == 8 || KPIId == 1000)
                {
