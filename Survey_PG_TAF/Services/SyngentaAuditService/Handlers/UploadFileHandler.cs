...
using SyngentaAuditService.Data;
...
namespace SyngentaAuditService.Handlers
{
    public class UploadFileHandler : AuthorizationHandler
    {
        ...
        public override HttpResponseMessage AuthorizationRequest()
        {
            ...
            if ("ATTENDANCE_ONLINE".Equals(FUNCTION, StringComparison.InvariantCultureIgnoreCase))
            {
                try
                {
                    ...
                    if (Context.Request.Files.Count > 0)
                    {
                        var file = Context.Request.Files[0];
                        ImageName = EmployeeId + "_" + ImageName.Replace(".product", ".jpeg").Replace(".query", ".jpeg");
                        string LinkPhoto = UrlServices + "/Upload/Images/" + ImageTime.Value.ToString("yyyyMMdd") + "/" + ImageName;
                        if (file != null)
                        {
                            if (file.ContentLength > 0)
                            {
                                ...
                                using (FileStream f = new FileStream(filename, FileMode.OpenOrCreate, FileAccess.ReadWrite))
                                {
                                    ...
                                    f.Flush();
                                    f.Close();
                                    using (DataSet ds = WorkController.Attendance_Action(EmployeeId, ShopId.Value, AttendanceType.Value, latitude, longitude, Accuracy, LinkPhoto, platform,version, 1))
                                    {

                                    }
                                    return HttpResponseMessage.CreateResponse(System.Net.HttpStatusCode.OK, "OK");
                                }
                            }
                            else
                            {
