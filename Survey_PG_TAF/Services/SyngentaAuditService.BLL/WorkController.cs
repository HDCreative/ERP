...
namespace SyngentaAuditService.BLL
{
    public static class WorkController
    {
        ...
        public static DataSet Attendance_Action(int EmployeeId, int ShopId, int AttendanceType, double? latitude, double? longitude, double? Accuracy, string LinkPhoto,string platform, string version, int ActionType)
        {
            using (var context = new WorkPlanContext())
            {
                return context.Attendance_Action(EmployeeId, ShopId, AttendanceType, latitude, longitude, Accuracy, LinkPhoto, platform,version, ActionType);
            }
        }
