...
class ShopController extends BaseController {
  ...
  Future<String> saveImage(WorkResultInfo work, ShopInfo shop, int type,
      File file, PickedFile pickedFile,int _attendantTime) async {
    error = "";
    ...
    try {
      ...
      if (file != null && file.existsSync() && await file.length() > 0) {
        ...
        photo.attendantTime = _attendantTime;// DateTimes.now();
  ...
  Future<void> startCamera(WorkResultInfo work, ShopInfo shop, int type) async {
    ...
    //   return;
    // }
    int __attendantTime = DateTimes.now();
    if (work != null && !work.locked) {
      ...
      try {
        ...
        if (pickedFile != null &&
          ...
          param["FUNCTION"] = "ATTENDANCE_ONLINE";
          param["ShopId"] = shop.shopId.toString();
          param["AttendanceType"] = type.toString();
          ...
          // param["ImageName"] = attendant.attendantPhoto
          //     .split("/")
          //     .last; /
          ...
          if (response.statusCode == 200) {
            await saveImage(work, shop, type, file, pickedFile,__attendantTime)
