...
import 'package:mutosi/app/base/AudioInfo.dart';
import 'package:mutosi/app/base/CoinCollectInfo.dart';
import 'package:mutosi/app/base/EvaluateResultInfo.dart';
import 'package:mutosi/app/base/MasterInfo.dart';
import 'package:mutosi/app/base/OsaResultInfo.dart';
import 'package:mutosi/app/base/PhotoInfo.dart';
import 'package:mutosi/app/base/PosmResult.dart';
import 'package:mutosi/app/base/ProductInfo.dart';
import 'package:mutosi/app/base/PromotionResult.dart';
import 'package:mutosi/app/base/SurveyAnswerInfo.dart';
import 'package:mutosi/app/base/SurveyDetailResultInfo.dart';
import 'package:mutosi/app/base/SurveyResultInfo.dart';
import 'package:mutosi/app/base/UploadInfo.dart';
import 'package:mutosi/app/base/UploadedItemInfo.dart';
import 'package:mutosi/app/context/photoContext.dart';
import 'package:mutosi/app/core/DateTimes.dart';
import 'package:mutosi/app/data/DatabaseHelper.dart';
import 'package:mutosi/app/data/TableNames.dart';
import 'package:mutosi/app/extensions/ExsString.dart';
import 'package:mutosi/app/models/AttendantInfo.dart';
import 'package:mutosi/app/models/OsaResultModel.dart';
import 'package:mutosi/app/models/PosmResultModel.dart';
import 'package:mutosi/app/base/ShopInfo.dart';
import 'package:mutosi/app/models/PromotionResultModel.dart';
import 'package:mutosi/app/models/SurveyDetailResultModel.dart';
import 'package:mutosi/app/models/SurveyResultModel.dart';
import 'package:mutosi/app/models/WorkResultInfo.dart';
...
class AuditContext {
  ...
  static Future<List<ProductInfo>> getAllProduct() async {
    String sql = "select * from Products order by sortlist desc";
    print("SQL: " + sql);
    try {
      var db = new DatabaseHelper();
      var data = await db.getList(sql);
      return data.map((e) => ProductInfo.fromMap(e)).toList();
    } catch (ex) {
      print("SQL-EXCEPTION: " + ex.toString());
      return null;
    }
  ...
  static Future<List<PromotionResultInfo>> getPromotionUpload(
      int workId) async {
  ...
  static Future<List<UploadedItemInfo>> getUploadItem(int workId) async {
    String sql = "select count(*) as itemId,'Attendants' as itemName from Attendants a where (a.uploaded is null or a.uploaded<>1) and a.workId=" +
        ...
        workId.toString() +
        " union all select count(*) as itemId,'Promotion' as itemName from " +
        TableNames.kpiPromotionResult +
        " p where p.workId=" +
        workId.toString();
    print("SQL: " + sql);
                ...
                if (haveResult == 1) {
                  ...
                  for (OsaResultModel osa in lstOsaTemp) {
                    if (!ExString(osa.keyBrand).isNullOrWhiteSpace()) {
                      if (osa.keyBrand != keyBrand) {
                        ...
                        osa.showKeyBrand = 1;
                        OsaResultModel osaBrand = OsaResultModel(
                            productId: osa.productId, keyBrand: osa.keyBrand);
                  ...
                  for (OsaResultModel osa in lstOsaTemp) {
                    if (ExString(osa.osaValue.toString())
                      ...
                      kpi.kpiStatus = 1;
                      break;
                    } else if (shop.shopType.endsWith("MT") &&
                        osa.osaValue > 0 &&
                        !osa.segment.endsWith("SURVEY")) {
                      if (ExString(osa.layerValue.toString())
                              .isNullOrWhiteSpace() ||
                          (osa.layerValue != null && osa.layerValue == 0)) {
                        kpi.kpiStatus = 1;
                        break;
                      }
                      if (!ExString(osa.footid.toString())
                        ...
                        if (ExString(osa.footValue.toString())
                                .isNullOrWhiteSpace() ||
                            (osa.footValue != null && osa.footValue == 0)) {
                          kpi.kpiStatus = 1;
                          break;
                        }
                      }
            ...
            if (kpi.id == 3) {
              ...
              if (lstTemp != null && lstTemp.length != 0) {
                for (PosmResultModel posm in lstTemp) {
                  ...
                  //   break;
                  // }
                  if (!ExString(posm.quantity.toString())
                          .isNullOrWhiteSpace() &&
                      posm.quantity > 0) {
                    haveResult = 1;
                    break;
                  ...
                  for (PromotionResultModel posm in lstTemp) {
                    ...
                    if (posm.value == 1) {
                      ...
                      if (lstPhotoByItem != null &&
                        ...
                        List<PhotoInfo> lstImg = lstPhotoByItem
                            .where(
                                (PhotoInfo e) => e.itemId == posm.promotionId)
                            .toList();
                        if (lstImg == null ||
                            (lstImg != null && lstImg.length == 0)) {
                          kpi.kpiStatus = 1;
  ...
  static Future<List<PromotionResultModel>> getPromotionResult(
    ...
    try {
      DatabaseHelper db = new DatabaseHelper();
      String sqlSelect =
          "select kp.shopId,kp.promotionId,kp.scheme,kpr.workId,kpr.value,kpr.comment,kpr.value1 from " +
              TableNames.kpiPromotion +
              " as kp " +
              "left join " +
              TableNames.kpiPromotionResult +
              " kpr on kp.promotionId = kpr.promotionId and kpr.workId = " +
              work.rowId.toString() +
              " where kp.shopId =" +
              work.shopId.toString();
      var data = await db.getList(sqlSelect);
      ...
      print("promoresult:" +
          data
              .map((Map e) => PromotionResultModel.fromMap(e).toJson())
              .toString());
    ...
    try {
      ...
      String sqlSelect =
          "select ka.productId,p.productName,ka.shopFormatId,ka.checkPrice,ka.targetOSA,p.milkPowder,p.keyBrand,p.brand,p.segment,p.photo as photo,p.sortlist,kas.rowId,kas.workId,kas.osaValue,kas.layerValue,kas.footValue, kas.comment,ml.id footid " +
              ...
              " ml on ml.listCode='MT_PR_C' and ml.refId=" +
              shop.shopId.toString() +
              " and ml.id = p.productId "
    ...
    try {
      ...
      String sqlSelect =
          "select p.keyBrand, p.segment,sum(kr.osavalue) osaValue  from KPIOSAResult kr  left join Products p on p.ProductId = kr.ProductId " +
              " where workId = " +
              work.rowId.toString() +
              " and p.segment <> 'SURVEY' and ifnull(kr.osaValue,0) >0 group by p.keyBrand, p.segment order by p.keyBrand, p.segment ";
      print(sqlSelect);
      var data = await db.getList(sqlSelect);
      List<OsaResultModel> lst =
          data.map((Map e) => OsaResultModel.fromMap(e)).toList();
    ...
    try {
      ...
      if (lst != null && lst.length != 0) {
        ...
        OsaResultInfo temp = new OsaResultInfo(
            ...
            layerValue: (item.osaValue != null && item.osaValue > 0
                ? item.layerValue
                : null),
            footValue: (item.osaValue != null && item.osaValue > 0
                ? item.footValue
                : null),
            workId: work.rowId);
        ...
        if (kpi.refId == 1 && (KPIId == 0 || KPIId == 1)) {
          ...
          if (kpi.id == 2 && (KPIId == 0 || KPIId == 2)) {
            ...
            for (OsaResultModel osa in lstOsaTemp) {
              if (!ExString(osa.keyBrand).isNullOrWhiteSpace()) {
                if (osa.keyBrand != keyBrand) {
                  keyBrand = osa.keyBrand;
                  osa.showKeyBrand = 1;
                  OsaResultModel osa_brand = OsaResultModel(
                      productId: osa.productId, keyBrand: osa.keyBrand);
            ...
            if (lstOsaTemp != null && lstOsaTemp.length != 0) {
              String keyBrand = "";
              for (OsaResultModel osa in lstOsaTemp) {
                if (ExString(osa.osaValue.toString()).isNullOrWhiteSpace()) {
                  return 'KPI ${kpi.name} - câu ${osa.productName} chưa hoàn thành.';
                } else if (osa.osaValue > 0 &&
                    shop.shopType.endsWith("MT") &&
                    !osa.segment.endsWith("SURVEY")) {
                  if (ExString(osa.layerValue.toString()).isNullOrWhiteSpace())
                  ...
                  // Có đk ks số chân
                  if (!ExString(osa.footid.toString()).isNullOrWhiteSpace()) {
                    if (ExString(osa.footValue.toString()).isNullOrWhiteSpace())
                ...
                if (osa.osaValue > 0) {
                  for (OsaResultModel osa_b_p in l_brand_photo) {
                    if (osa.keyBrand == osa_b_p.keyBrand) osa_b_p.osaValue = 1;
                  }
                }
              ...
              for (OsaResultModel osa in lstOsaTemp) {
                ...
                for (OsaResultModel osa in l_brand_photo) {
                  if (!ExString(osa.osaValue.toString()).isNullOrWhiteSpace() &&
                    ...
                    if (lstImg == null ||
                        (lstImg != null && lstImg.length == 0)) {
                      return 'KPI ${kpi.name} - Brand ${osa.keyBrand} chưa chụp đủ hình theo yêu cầu 0/1.';
                    }
                  }
                }
              }
          ...
          if (kpi.id == 3 && (KPIId == 0 || KPIId == 3)) {
            ...
            if (lstTemp != null && lstTemp.length != 0) {
              for (PosmResultModel posm in lstTemp) {
                ...
                if (ExString(posm.quantity.toString()).isNullOrWhiteSpace()) {
                  return 'KPI ${kpi.name} - câu ${posm.posmName} chưa hoàn thành.';
                }
                if (posm.value == 1) {
