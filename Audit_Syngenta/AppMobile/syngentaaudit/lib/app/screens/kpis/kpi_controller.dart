...
class KPIController extends BaseController {
  ...
  Future<void> getDataOsaSummary(
      ShopInfo shop, WorkResultInfo work, MasterInfo kpi) async {
    await showProgess();
    List<OsaResultModel> lstTemp  = await AuditContext.getOsaResult_Summary(work, shop);
    String keyBrand ="";
    if (lstTemp != null && lstTemp.length != 0) {
      int facingTotal = 0;
      for (OsaResultModel item in lstTemp) {
  ...
  Future<void> setOOSAll(
    ...
    await lock.synchronized(() async {
          ...
          if(item.value.osaValue == null)
            {
              ...
              await lock.synchronized(() async {
                await AuditContext.setDataOsa(work, item.value).then((String value) {
                  if (ExString(value).isNullOrWhiteSpace()) {
                    lstOsa.refresh();
                  } else {
                    alert(content: value);
      ...
      if(hasUpdate == true) {
        lstOsaFilter.value = lstOsa;
        print(lstOsaFilter[20].value.osaValue.toString() + "-" + lstOsaFilter[20].value.productName);
      }
  ...
  Future<void> searchDataOsa() async{
    if(ExString(selectProductOSA).isNullOrWhiteSpace()){
      lstOsaFilter.value = lstOsa;
      if(!ExString(selectKeyBrandOSA.value).isNullOrWhiteSpace() && selectKeyBrandOSA.value.toLowerCase() != 'tất cả'){
        lstOsaFilter.value = lstOsaFilter.where((osa) => osa.value.keyBrand == selectKeyBrandOSA.value).toList();
      }
      if(eNotDoneOSA.value){
        lstOsaFilter.value = lstOsaFilter.where((osa) => ExString(osa.value.osaValue.toString()).isNullOrWhiteSpace()).toList();
      }
    }else{
      lstOsaFilter.value = lstOsa.where((osa) => osa.value.productName.toLowerCase().contains(selectProductOSA,0)).toList();
      if(!ExString(selectKeyBrandOSA.value).isNullOrWhiteSpace() && selectKeyBrandOSA.value.toLowerCase() != 'tất cả'){
        lstOsaFilter.value = lstOsaFilter.where((osa) => osa.value.keyBrand == selectKeyBrandOSA.value).toList();
      }
      if(eNotDoneOSA.value){
