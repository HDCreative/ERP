...
class ShopDetailState extends State<ShopDetail> {
              ...
              children: [
                ...
                Align(
                  ...
                  child: Container(
                    child: Row(
                      children: [
                        Text(shop != null && shop.contactName.isNullOrWhiteSpace()
                            ? ''
                            : shop.contactName),
                      ],
                    ),
  ...
  Future<void> startCamera() async {
    ...
    // if (gps.isMocked) {
    //   controller.alert(content: 'Bạn đang sử dụng giả lập GPS. Vui lòng tắt ứng dụng giả lập !');
    //   return ;
    // }
    try {
    ...
    if (controller.overview.value == null) {
      ...
      if (!ExString(controller.overview.value.attendantPhoto)
        ...
        if (file != null && file.existsSync() && file.lengthSync() > 0) {
          return Image.file(
            new File(controller.overview.value.attendantPhoto),
            fit: BoxFit.cover,
