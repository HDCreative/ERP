          ...
          for (UploadInfo item in dataUpload) {
            ...
            if (listAttendant != null && listAttendant.length > 0) {
              for (AttendantInfo attendant in listAttendant) {
                File file = new File(attendant.attendantPhoto);
                if (file != null &&
                    file.existsSync() &&
                    file.lengthSync() > 0) {
                  var map = <String, dynamic>{};
                  map["ShopId"] = attendant.shopId;
                  map["ImageName"] = file.path.split("/").last;
                  ...
                  map["Accuracy"] = attendant.accuracy;
                  map["AttendantServer"] =
                      attendant.getUrl(loginInfo.employeeId);
            ...
            if (listPhoto != null && listPhoto.length > 0) {
              for (PhotoInfo photo in listPhoto) {
                ...
                if (file != null &&
                  ...
                  map["PhotoName"] = file.path.split("/").last;
                  map["PhotoType"] = photo.photoType;
                  map["PhotoTime"] = new DateFormat("yyyy-MM-dd HH:mm:ss")
                      .format(new DateTime.fromMillisecondsSinceEpoch(
                          photo.photoTime));
            ...
            if (listAudio != null && listAudio.length > 0) {
              for (AudioInfo audio in listAudio) {
                File file = new File(audio.audioLocal);
                if (file != null &&
                    file.existsSync() &&
                    file.lengthSync() > 0) {
                  var map = <String, dynamic>{};
                  map["AudioName"] = file.path.split("/").last;
            ...
            if (response.statusCode == 200) {
              if (listAttendant != null && listAttendant.length > 0) {
                for (AttendantInfo att in listAttendant) {
                  File file = new File(att.attendantPhoto);
                  if (file != null &&
                      file.existsSync() &&
                      file.lengthSync() > 0) {
