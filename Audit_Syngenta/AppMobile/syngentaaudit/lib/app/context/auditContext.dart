...
class AuditContext {
  static Future<List<UploadInfo>> getUpload() async {
    String sql =
        "select s.comment,s.rowId as workId, s.shopId,s.shopName,s.workDate,s.workTime,s.shopFormatId,s.auditResult,shopType from WorkResult s where (s.isUpload is null or s.isUpload <> 1) and s.isDone=1 and s.shopId in (select shopId from Attendants where attendantType = 1) ORDER BY s.workDate DESC";
    print("SQL: " + sql);
    try {
  ...
  static Future<List<OsaResultModel>> getOsaResult(
    ...
    try {
      DatabaseHelper db = new DatabaseHelper();
      String sqlSelect =
          "select ka.productId,p.productName,ka.shopFormatId,ka.checkPrice,ka.targetOSA,p.milkPowder,p.keyBrand,p.brand,p.segment,p.photo as photo,p.sortlist,kas.rowId,kas.workId,kas.osaValue,kas.comment " +
              "from KPIOSA ka " +
  ...
  static Future<String> checkData(
      {WorkResultInfo work, ShopInfo shop, String listCode = "KPI"}) async {
    var db = new DatabaseHelper();
    String sql = "select * from MasterList where listCode = '" +
        listCode +
        "' and refCode = '" +
