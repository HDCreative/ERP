...
namespace HR_Grove_Web.App_Code
{
    public class Pf
    {
        public static string OptionChooseEN = "-All-";
        public static string OptionChooseVN = "-Tất cả-";
        ...
        public static void bindOfficeDropDown(int UserId, int? AreaId, int? ProvinceId, int? DistrictId, int? TownId, string Value, string Text, ref DropDownList ddl)
        {
            DataTable data = new StoreListController().StoreListGetList(UserId, null, AreaId, ProvinceId, DistrictId, TownId, null, null, 1, 200000);
            var data_tmp = data.AsEnumerable().Select(x => new { Value = x[Value], Text = x[Text] }).Distinct().ToList();
            if (data != null && data.Rows.Count > 0)
            {
                ddl.DataTextField = "Text";
