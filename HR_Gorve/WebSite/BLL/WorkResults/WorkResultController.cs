...
namespace BLL.WorkResults
{
    public class WorkResultController
    {
            ...
            int? PageNumber, int? RowNumber)
        {
            ...
            {
                return context.WorkResultGetList(LoginId, FromDate, ToDate, SupId, Auditor, AuditResult, ShopCode, AreaId, ProvinceId, DistrictId, TownId, MVOId, POGId, QCStatus, LWorkId, PageNumber, RowNumber);
            }
        }
        ...
        public DataSet ReportAttendance(int LoginId, DateTime FromDate, DateTime ToDate)
        {
            using (var context = new WorkResultsContext())
            {
                return context.ReportAttendance(LoginId, FromDate, ToDate);
            }
        }
        ...
        public DataTable WorkResultGetKPIPOSM(long WorkId, int ShowAll = 0)
        {
            using (var context = new WorkResultsContext())
            {
                return context.WorkResultGetKPIPOSM(WorkId, ShowAll);
            }
        }
           ...
           int? PageNumber, int? RowNumber)
        {
            using (var context = new WorkResultsContext())
            {
                return context.BillResultGetList(LoginId, FromDate, ToDate, AreaId, ProvinceId, DistrictId, TownId, Site, ShopCode, Mobile, ProductCode, BillSeri, PageNumber, RowNumber);
            }
            ...
            int AreaId, int ProvinceId, int DistrictId, int TownId, int? Site, string ShopCode, string Mobile, string ProductCode, string BillSeri)
        {
            using (var context = new WorkResultsContext())
            {
                return context.BillResultExport(LoginId, FromDate, ToDate, AreaId, ProvinceId, DistrictId, TownId, Site, ShopCode, Mobile, ProductCode, BillSeri);
            }
