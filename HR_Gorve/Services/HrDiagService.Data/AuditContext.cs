...
{
    public class AuditContext : BaseDataContext
    {
        ...
        public IEnumerable<MasterInfo> MasterGetList([Parameter(Name = "@EmployeeId", DbType = "INT")]int EmployeeId,
            [Parameter(Name = "@ListCode", DbType = "VARCHAR(128)")] string ListCode)
        {
            var result = this.ExecuteMethodCall(this, (MethodInfo)MethodBase.GetCurrentMethod(), EmployeeId, ListCode);
        ...
        public DataSet GetBill(int EmployeeId, int? ShopId, string CusName, string BillSeri, int? FromDate, int? ToDate)
        {
            var result = SqlHelper.ExecuteDataset("[dbo].[Mobile.GetBill]", ShopId, CusName, BillSeri, FromDate, ToDate);
            return result;
        }
    }
}
