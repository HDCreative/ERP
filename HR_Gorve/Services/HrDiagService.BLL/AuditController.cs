...
namespace HrDiagService.BLL
{
    public class AuditController
    {
        ...
        public static List<MasterInfo> MasterGetList(int EmployeeId,string ListCode)
        {
            List<MasterInfo> _lst = null;
            using (AuditContext context = new AuditContext())
            {
                var value = context.MasterGetList(EmployeeId, ListCode);
                if (value != null)
                    _lst = value.ToList();
            }
            return _lst;
        ...
        public static DataSet GetBill(int EmployeeId, int? ShopId, string CusName, string BillSeri, int? FromDate, int? ToDate)
        {
            DataSet _lst = null;
            using (AuditContext context = new AuditContext())
            {
                var value = context.GetBill(EmployeeId, ShopId, CusName, BillSeri, FromDate, ToDate);
                _lst = value;
            }
            return _lst;
        }

        public static int InsertLocation(int employeeId, string latitude, string longitude, string accuracy)
        {
            try
            {
                ...
                command.Parameters.Add(new SqlParameter("@Accuracy", accuracy));
                command.CommandTimeout = 5;
                return DAUltility.Helpers.SqlHelper.excuteSqlCommnd(command, "[dbo].[Mobile.Insert.UserLocation]");
            }
            catch (Exception ex)
            {
                throw ex;
